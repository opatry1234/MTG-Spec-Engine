...
def archived_theme_cards(slug: str, anchor: date) -> dict[str, dict]:
    """name -> {synergy, inclusion, num_decks, ...} from the newest pre-anchor
    snapshot of an EDHREC page, PLUS '__commanders__' (theme pages only): the
    page's top commander slugs — their own archived pages carry the DEEP staple
    lists (theme pages truncate to ~10 cards per category; spikers live deeper).
    ``slug`` is a themes/ slug or a full path (typal/zombies, commanders/<name>).
    Cached on disk; {} on any failure."""
    ...
    try:
        ...
        if cards and not path.startswith("commanders/"):
            cards["__commanders__"] = _parse_commander_slugs(html)  # type: ignore[assignment]
        if cards:
...
def theme_staple_scores(
    ...
    queue = [s for mid in mechanic_ids for s in MECHANIC_THEME_SLUGS.get(mid, [])]
    queue += tribal_slugs(context_text)
    while queue:
        slug = queue.pop(0)
        if slug in seen_slugs:
            continue
        seen_slugs.add(slug)
        page = archived_theme_cards(slug, anchor)
        # Theme pages truncate to ~10 cards/category — recurse one level into the
        # theme's top commanders' own pages, where the deep staple lists live.
        for cs in page.pop("__commanders__", []) or []:
            queue.append(f"commanders/{cs}")
        for name, v in page.items():
